...
#order the collectiosn by order of reverse numbre of collections connections
...
for ($i=count($ord)-1; $i >=0 ; $i=$i-1)  {
	  ...
	  echo '<div id="'.$subject_names[$collection].'" style="position:relative;"><table width = "100%" id="'.$collection.'_table" border>
			<tr style="background-color:#D5EDB3">
				...
				<td id="'.$collection.'_td"><input type="text" class="'.$collection.'_item" name="'.$subject_names[$collection].'" id="'.$collection.'" rules="'.$ruleObject.'" objectrules="'.$ruleObject.'" onkeydown="findItem(\''.$collection.'\',document.getElementById(\''.$collection.'\').value, \''.$collection.'_link\',event)">
				<span id="'.$collection.'_lastItem">(Last inserted: processing...)</span><br />
				<span style="font-style: italic; color: #330066; text-decoration: underline" id="'.$collection.'_link" onClick="findItem(\''.$collection.'\',document.getElementById(\''.$collection.'\').value, \''.$collection.'_link\')"><br /></span>
